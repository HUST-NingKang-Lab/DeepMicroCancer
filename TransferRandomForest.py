...
def predict(RF, X):
    ypred = np.array(list(map(lambda x: forest_classify_ensemble(RF, x), X)))
    return ypred

def mix_predict(SER_RF, STRUT_RF, X):

    ypred = []
    
    for p in range(X.shape[0]):
        PSER = forest_posterior(SER_RF, X[p]).mean(axis=0)
        PSTRUT = forest_posterior(STRUT_RF, X[p]).mean(axis=0)
        PMIX = np.array((PSER, PSTRUT))
        ypred.append(np.argmax(PMIX.mean(axis=0)))
    ypred = np.array(ypred)

    return ypred

def predict_proba(RF, X):

    y_proba = []

    for n in range(X.shape[0]):
        Pforest = forest_posterior(RF, X[n]).mean(axis=0)
        y_proba.append(Pforest)
    y_proba = np.array(y_proba)

    return y_proba

def mix_predict_proba(SER_RF, STRUT_RF, X):

    y_proba = []

    for n in range(X.shape[0]):
        PSER = forest_posterior(SER_RF, X[n]).mean(axis=0)
        PSTRUT = forest_posterior(STRUT_RF, X[n]).mean(axis=0)
        y_proba.append(np.array((PSER, PSTRUT)).mean(axis=0))
    y_proba = np.array(y_proba)
